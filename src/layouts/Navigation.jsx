...
export default function Navigation () {
  ...
  const navLinks = mobileLinks.slice(1, 4);
  const profile = useSelector((state) => state.user?.profile);
  const items = useSelector((state) => state.cart.items);

  useEffect(() => {
    if (profile?._id) dispatch(fetchCart());
      ...
      <div className="max-w-[1550px] mx-auto px-4 md:px-8">
        <div className="flex justify-between items-center h-24">
              ...
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    key={link.label}
                    to={link.href}
                    className="text-[15px] font-medium text-black transition-all duration-300 relative py-1"
                  >
                    {link.label}
                    {/* Persistent Active Underline for Desktop */}
                    {isActive ? (
                      <motion.span
                      ...
                      <span
                        className="absolute bottom-0 left-0 w-0 h-[1px] bg-black/40 transition-all duration-500 hover:w-full"/>
                    )}
                  </Link>
                );
              })}
          ...
          <div className="flex items-center gap-4 md:hidden">
            ...
            <button onClick={() => setIsOpen(!isOpen)} className="z-[100] p-2 text-black active:scale-90">
                  ...
                  return (
                    <div key={link.label} className="overflow-hidden">
                      ...
                      >
                        <Link
                          to={link.href}
                          className={`relative inline-flex items-baseline gap-4 text-5xl font-black tracking-tighter transition-all duration-300 
                            ${isActive ? 'text-black' : 'text-neutral-400'}`}
                        >
                          {link.label}
                          {/* Bold Blue Underline for Mobile Active Link */}
                          {isActive && (
                            <motion.div
                              layoutId="mobileNavUnderline"
                              className="absolute -bottom-2 left-0 w-full h-2 bg-blue-600 -z-10"
                            />
                          )}
                          {link.showCount && items?.length > 0 && (
                            <span className={`text-2xl font-medium ${isActive ? 'text-black' : 'text-neutral-400'}`}>
                              ({items.length})
                            </span>
                          )}
                        </Link>
