        ...
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {isCollapsed && (
                <div
                  className="fixed left-20 ml-4 px-3 py-1.5 bg-slate-800 text-white text-xs font-bold rounded-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-all duration-200 border border-slate-700 shadow-xl translate-x-[-10px] group-hover:translate-x-0">
                  {item.label}
                  {/* Tooltip Arrow */}
                  <div
                    className="absolute top-1/2 left-[-4px] -translate-y-1/2 w-2 h-2 bg-slate-800 border-l border-b border-slate-700 rotate-45"/>
                </div>
              )}
            </button>
          );
